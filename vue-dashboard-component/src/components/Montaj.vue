        ...
        <v-card-title primary-title>
            <div>
                ...
                <v-checkbox label="Использовать секцию"
                            v-model="product.montaj.isUsed"
                            :input-value="product.montaj.isUsed"
                            :false-value="false"
                            :true-value="true"
                            color="success"/>
            </div>
        ...
        <v-card-text>
            ...
            <div v-if="product.montaj.isUsed">
                ...
                <v-text-field label="Название шага *"
                              v-model="product.montaj.step"
                              hint="заголовок который будет отображен как номер шага"/>

                <v-text-field label="Заголовок шага *"
                        v-model="product.montaj.name"
                        hint="заголовок который будет отображен как название шага"/>

                <v-layout v-for="(item, index) in product.montaj.items" :key="index">
                    <v-flex md4>
                        <v-text-field
                                label="Название опции *"
                                v-model="product.montaj.items[index].name"
                                hint="название опции"/>
                    </v-flex>
                    ...
                    <v-flex md4 class="mx-3">
                        <v-text-field
                                ...
                                v-model="product.montaj.items[index].price"
                                hint="цена за опцию"/>
                    </v-flex>

                    <v-flex md4>
                        <v-text-field label="Фото тамб"
                                      v-model="product.montaj.items[index].thumb"
                                      :hint="product.base.nameDB ? `${product.base.nameDB}/${sectionName}/thumb/имя-фото.jpg` : '!!Введите название продукта (шаг 1)'"/>
                    </v-flex>

                    <v-btn  icon color="primary" flat @click="openDialog(index)" :disabled="index === 0">x</v-btn>
                ...
                <v-textarea name="input-7-1"
                            label="Описание продукта (опционально)"
                            v-model="product.montaj.description"
                            hint="Описание продукта"/>
            </div>
        </v-card-text>
    </v-card>
</template>
...
export default {
  ...
  methods: {
    addItem () {
      this.product.montaj.items.push(Object.assign({}, this.itemDefault))
    },
    deleteItem () {
      this.product.montaj.items.splice(this.currentIndex, 1)
      this.dialog = false
    },
    openDialog (index) {
